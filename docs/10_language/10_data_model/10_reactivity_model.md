...
In actual implementations, intrinsic types typically **do not store** identity, revision
and status information explicitly:
...
3. **Status**: Availability state (Loading/Ready/Error) that propagates through dependencies; status
   transitions are structural changes
...
- **Status-aware computation**: Track data availability and propagate loading/error states automatically

The model forms the conceptual foundation for how changes are tracked and updates propagate
